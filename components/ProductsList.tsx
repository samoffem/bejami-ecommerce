...
interface Props {
    ...
    handleSort: (e:HTMLSelectElement)=>void,
    setOrder: (p:string | ((prev:string)=>string))=>void,
...
const ProductsList: React.FC<Props> = ({products, handleCategoryChange, handlePriceChange, handleSort, setOrder}) => {
                    ...
                    <div className={`${styles.categories} ${toggle? styles.show:''}`}>
                        <div className={styles.categoriesWrapper}>
                            <div className={styles.categoriesList}>
                                ...
                                {categories.map((item, ind)=>(
                                    <div key={ind} className={styles.item}>
                                        <input onChange={handleChange} type="checkbox" value={item.name} />
                                        <span>{item.name}</span>
                                    </div>
                                ))}
                            </div>
                            <div className={styles.priceRange}>
                                ...
                                {prices.map((item, ind)=>(
                                    <div key={ind} className={styles.item}>
                                        <input onChange={handlePrices} type="radio" name='price' value={item} />
                                        <span>{item}</span>
                                    </div>
                                ))}
                            </div>
                            ...
                            {products.map((product, index)=>(
                                <div key={index} className={styles.product}>
                                    <div className={styles.productImg}>
                                        <Image src={product.image.src} layout="fill" alt={product.image.alt} />
                                        <div onClick={()=>cartDispatch({type:'ADD_TO_CART', payload: product})} className={styles.addToCart}>ADD TO CART</div>
                                        {product.bestSeller && 
